...
**When to Use**: After building or modifying any screen — especially with `/visual-review`
...
Reviews Flutter screens against the systematic grid and typography principles from Müller-Brockmann's canonical work, adapted for mobile/tablet/desktop app design. The grid creates "a sense of compact planning, intelligibility and clarity, and suggests orderliness of design."
...
**Check:** Card internal padding (16dp) should be <= gap between cards (16dp or more). If a card has 16dp padding but cards are only 8dp apart, the layout feels cramped and items don't read as distinct groups.
...
- Cards with text don't become too wide on landscape/desktop
...
4. **Consistent sizing** — similar content uses identical dimensions (all avatar = 40dp, all thumbnails = 80dp)
...
1. **White space creates grouping** — more space between unrelated items, less between related items (Gestalt proximity)
2. **Margins are generous** — resist the urge to fill every pixel
3. **Consistent rhythm** — spacing increases in predictable steps (16 → 24 → 32), never random jumps (16 → 19 → 28)
...
Use `/visual-review` to capture the screen at phone (390x844), tablet (768x1024), and desktop (1200x800) viewports.

### Step 2: Grid Overlay Analysis
Mentally overlay a 4-column (phone) or 8-column (tablet) grid. Check:
- Do elements align to column edges?
- Are gutters consistent?
- Do margins match across screens?

### Step 3: Measure Spacing
Scan the widget tree for spacing values. Flag:
