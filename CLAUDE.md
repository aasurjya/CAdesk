...
- Dart strict mode, no `dynamic` types without justification
- flutter_lints (analysis_options.yaml)
- All quality gates must pass before PR creation

## Key Decisions

- **Platform**: Flutter (Dart) targeting iOS (iPhone + iPad), macOS, and Web
- **Architecture**: Clean Architecture (presentation / domain / data layers)
- **State Management**: Riverpod (immutable state, provider-based)
- **Navigation**: GoRouter (declarative routing)
- **Database**: Drift (SQLite) for local, Supabase/Firebase for cloud sync
- **UI**: Material 3 with adaptive layouts for phone/tablet/desktop
...
- File size limit: 400 lines typical, 800 max
