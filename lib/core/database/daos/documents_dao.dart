...
class DocumentsDao extends DatabaseAccessor<AppDatabase> with _$DocumentsDaoMixin {
  ...
  Stream<List<DocumentRow>> watchDocumentsByClient(String clientId) =>
      (select(documentsTable)..where((t) => t.clientId.equals(clientId))).watch();
  ...
  Future<DocumentRow?> getDocumentById(String documentId) =>
      (select(documentsTable)..where((t) => t.id.equals(documentId)))
          .getSingleOrNull();
  ...
  Future<bool> deleteDocument(String documentId) async {
    final result = await (delete(documentsTable)..where((t) => t.id.equals(documentId))).go();
    return result > 0;
  }
  ...
  Future<List<DocumentRow>> searchDocuments(String query) {
    ...
    return (select(documentsTable)
          ..where(
            (t) => t.title.like(q) | t.tags.like(q),
          ))
        .get();
  }
}
