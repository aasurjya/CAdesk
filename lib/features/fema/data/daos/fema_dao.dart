...
class FemaDao extends DatabaseAccessor<AppDatabase> with _$FemaDaoMixin {
  ...
  Future<String> insertFemaFiling(FemaFilingsTableCompanion companion) async {
    ...
    final rows = await (select(femaFilingsTable)
          ..orderBy([
            (t) => OrderingTerm(
  ...
  Future<List<FemaFilingsTableData>> getFemaFilingsByClient(
    String clientId,
  ) =>
  ...
  Future<List<FemaFilingsTableData>> getFemaFilingsByType(
    String filingType,
  ) =>
      (select(femaFilingsTable)
  ...
  Future<FemaFilingsTableData?> getFemaFilingById(String id) =>
      (select(femaFilingsTable)..where((t) => t.id.equals(id)))
          .getSingleOrNull();
  ...
  Future<bool> updateFemaFilingStatus(String id, String status) async {
    final count = await (update(femaFilingsTable)
