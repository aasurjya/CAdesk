...
import '../data/providers/ecosystem_providers.dart';
import '../domain/models/integration_connector.dart';
import 'widgets/connector_tile.dart';
import 'widgets/marketplace_app_tile.dart';
...
class _EcosystemScreenState extends ConsumerState<EcosystemScreen>
  ...
  Widget build(BuildContext context) {
    final summary = ref.watch(ecosystemSummaryProvider);

    return Scaffold(
      backgroundColor: AppColors.neutral50,
      appBar: AppBar(
        title: const Text('Ecosystem & Integrations'),
        centerTitle: false,
          ...
          Padding(
            ...
            child: Row(
              children: [
                ...
                _SummaryCard(
                  label: 'Connected',
                  count: summary.connectedConnectors,
...
class _ConnectorsTab extends ConsumerWidget {
  const _ConnectorsTab();

  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final connectors = ref.watch(filteredConnectorsProvider);
    final selectedStatus = ref.watch(connectorStatusFilterProvider);

    return Column(
      children: [
        // Status filter chips
        _StatusFilterBar<ConnectorStatus>(
          ...
          child: connectors.isEmpty
              ...
              : ListView.builder(
                  ...
                  itemBuilder: (context, index) =>
                      ConnectorTile(connector: connectors[index]),
                ),
        ),
      ],
    );
...
class _MarketplaceTab extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return apps.isEmpty
        ? const _EmptyState(message: 'No apps available in the marketplace')
        : ListView.builder(
            padding: const EdgeInsets.only(top: 8, bottom: 80),
            itemCount: apps.length,
            itemBuilder: (context, index) =>
                MarketplaceAppTile(app: apps[index]),
          );
  }
}
